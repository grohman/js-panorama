...
b.NumberOfKeys(b.opts.eventsSettings.lastXYById))){g.preventDefault&&g.preventDefault();g.preventManipulation&&g.preventManipulation();g.preventMouseEvent&&g.preventMouseEvent();for(var e=g.changedTouches?g.changedTouches:[g],h=0;h<e.length;++h){var l=e[h],j="undefined"!=typeof l.identifier?l.identifier:"undefined"!=typeof l.pointerId?l.pointerId:1;b.EnsurePageXY(l);var k=l.pageX,l=l.pageY;if(g.type.match(/(start|down)$/i))documentToTargetDelta=b.ComputeDocumentToElementDelta(a),b.opts.eventsSettings.lastXYById[j]&&
(delete b.opts.eventsSettings.lastXYById[j],b.opts.debug&&c("<div>").html("end").prependTo(b.opts.debug)),b.opts.debug&&c("<div>").html("start at "+k+"px").prependTo(b.opts.debug),b.opts.eventsSettings.lastXYById[j]={x:k,y:l},f=k,a.msSetPointerCapture?a.msSetPointerCapture(j):"mousedown"==g.type&&1==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)&&(b.opts.eventsSettings.useSetReleaseCapture?a.setCapture(!0):(document.addEventListener("mousemove",d,!1),document.addEventListener("mouseup",d,!1)));
else if(g.type.match(/move$/i))b.opts.eventsSettings.lastXYById[j].x=k,b.opts.eventsSettings.lastXYById[j].y=l,m=k-f,j=20,Math.abs(m)>j?(0>m&&(j=-j),j=m-j,b.opts.debug&&c("<div>").html("move: "+j+"px").prependTo(b.opts.debug),b.getItemsContainer().css("left",j+"px")):m=0;else if(b.opts.eventsSettings.lastXYById[j]&&g.type.match(/(up|end|cancel)$/i)){b.opts.debug&&c("<div>").html("end").prependTo(b.opts.debug);var n=setInterval(function(){!1==b.opts.busy&&clearInterval(n);0<m?b.goToPrevious():0>m&&
b.goToNext();m=0},10);delete b.opts.eventsSettings.lastXYById[j];a.msReleasePointerCapture?a.msReleasePointerCapture(j):"mouseup"==g.type&&0==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)&&(b.opts.eventsSettings.useSetReleaseCapture?a.releaseCapture():(document.removeEventListener("mousemove",d,!1),document.removeEventListener("mouseup",d,!1)))}}}}var b=this;if(window.navigator.msPointerEnabled)a.addEventListener("MSPointerDown",d,!1),a.addEventListener("MSPointerMove",d,!1),a.addEventListener("MSPointerUp",
...
(a?c("<div>").html("swipe left").prependTo(this.opts.debug):c("<div>").html("swipe right").prependTo(this.opts.debug));var d,b,f=this.getItem(this.opts.currentIndex);b=this.fixIndex(a?this.opts.currentIndex+1:this.opts.currentIndex-1);d=this.fixIndex(a?this.opts.currentIndex+2:this.opts.currentIndex+1);var e=this.getItem(b),g=2<this.totalNumberOfItems?this.getItem(d):void 0,h,k;!a&&e&&(e.css({left:-this.opts._item_width+this.opts.itemsLeftMarginPercent}),3<this.totalNumberOfItems&&(h=this.fixIndex(b-
1),k=this.getItem(h),this.getItems().not(":eq("+this.opts.currentIndex+")").not(":eq("+b+")").not(":eq("+h+")").hide(),this.getItem(this.fixIndex(b-1)).css({left:-this.opts._item_width+this.opts.itemsLeftMarginPercent})));a&&g&&(g.css({left:2*this.opts._item_width+this.opts.itemsLeftMarginPercent}),this.getItems().not(":eq("+this.opts.currentIndex+")").not(":eq("+d+")").hide());a={left:(a?"-=":"+=")+this.opts._item_width};e&&e.show();g&&g.show();var l=this;e&&e.animate(a,this.opts.animationDuration,
this.opts.animationEasing,function(){l.opts.currentIndex=b;l.opts.busy=!1;k&&k.show();l.$el.trigger(l.opts.selectedItemChangedEventName)});g&&g.animate(a,this.opts.animationDuration,this.opts.animationEasing);f.animate(a,this.opts.animationDuration,this.opts.animationEasing);this.getItemsContainer().animate({left:"0px"},this.opts.anumationDuration,this.opts.animationEasing);this.opts.titleSelector&&this.getTitle().animate({marginLeft:-b*this.opts.titleAnimationStep+"%"},this.opts.animationDuration,
this.opts.animationEasing);this.$el.animate({backgroundPositionX:b*this.opts.backgroundAnimationStep+"%"},this.opts.animationDuration,this.opts.animationEasing);return!0}}k.prototype={init:function(){var c=this.$el;if(c.data("panorama"))return!1;c.attr("data-panorama",!0);this.prepare(c);return!0},prepare:function(f){var e=f.parent().height()+1;f.height(e);var h=this.getItemsContainer().position().top,a=e-h;this.getItemsContainer().height(a);this.totalNumberOfItems=this.getItems().length;var d=f.outerWidth()*
(100-this.opts.itemsGapPercentage)/100;this.opts._item_width=d;f=Math.round(c(window).width()/100*this.opts.itemsLeftMarginPercent);this.opts.itemsLeftMarginPercent=f;this.getItems().each(function(b,e){c(e).height(a-(c(e).outerHeight()-c(e).height()));c(e).width(d-(c(e).outerWidth()-c(e).width()));1==b&&c(e).css({left:d});1<b&&c(e).hide()});2<this.opts.totalNumberOfItems&&(this.getLastItem().css({left:"-"+d+"px"}).show(),this.getItems().css("left","+="+f+"px"))},show:function(){var f=this;this.getItemsContainer().fadeIn(function(){var e=
f.getItemsContainer().position().top;f.getItems().each(function(){c(this).css("height","-="+e+"px")})})}};c.fn.panorama=function(f){this.length&&this.each(function(){var e=new k(this,f);e.init();e.setEvents(this);e.show();c(this).data("panorama",e)})}})(jQuery);$.easing.easeOutExpo=function(c,k,f,e,h){return k==h?f+e:e*(-Math.pow(2,-10*k/h)+1)+f};