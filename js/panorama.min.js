...
d.extend(this.opts,{currentIndex:0,busy:!1,_item_width:0,eventsSettings:{useSetReleaseCapture:!1,lastXYById:{}},debug:!0==this.opts.debug&&d("<div>").css({position:"fixed",bottom:"0px",right:"0px",background:"white",border:"1px solid red",overflow:"hidden",width:"200px",height:"20%",zIndex:"9999"}).appendTo("body")});0==parseInt(this.opts.backgroundAnimationStep)&&(this.opts.backgroundAnimationStep=Math.round(100/d(e).find(this.opts.itemSelector).length));0==parseInt(this.opts.totalNumberOfItems)&&
(this.opts.totalNumberOfItems=d(e).find(this.opts.itemSelector).length);if(this.opts.debug){var h=d("<div>").css({position:"fixed",bottom:"0px",left:"0px",background:"white",border:"1px solid red"}).appendTo("body");d("<h2>").html("<a href=\"#\" onclick=\"$('.ui-panorama').data('obj').goToNext();\">swipe left</a>").appendTo(h);d("<h2>").html("<a href=\"#\" onclick=\"$('.ui-panorama').data('obj').goToPrevious();\">swipe right</a>").appendTo(h);d("<h2>").html("<a href=\"#\" onclick=\"$('.ui-panorama').data('obj').goTo(3);\">go to screen 3</a>").appendTo(h);
d("<h2>").html("<a href=\"#\" onclick=\"alert($('.ui-panorama').data('obj').opts.currentIndex);\">get current idx</a>").appendTo(h)}this.$el=d(e);this.getItemsContainer=function(){return this.$el.children(this.opts.itemsContainerSelector)};this.getItems=function(){return this.getItemsContainer().children(this.opts.itemSelector)};this.getItem=function(a){return this.getItemsContainer().children(this.opts.itemSelector+":eq("+a+")")};this.getLastItem=function(){return this.getItem(this.opts.totalNumberOfItems-
...
this.goToPrevious=function(){this.changeItem(!1)};this.setEvents=function(a){function c(g){if(!("mousemove"==g.type&&0==b.NumberOfKeys(b.opts.eventsSettings.lastXYById))){g.preventDefault&&g.preventDefault();g.preventManipulation&&g.preventManipulation();g.preventMouseEvent&&g.preventMouseEvent();for(var f=g.changedTouches?g.changedTouches:[g],h=0;h<f.length;++h){var l=f[h],j="undefined"!=typeof l.identifier?l.identifier:"undefined"!=typeof l.pointerId?l.pointerId:1;b.EnsurePageXY(l);var k=l.pageX,
l=l.pageY;if(g.type.match(/(start|down)$/i))documentToTargetDelta=b.ComputeDocumentToElementDelta(a),b.opts.eventsSettings.lastXYById[j]&&(delete b.opts.eventsSettings.lastXYById[j],b.opts.debug&&d("<div>").html("end").prependTo(b.opts.debug)),b.opts.debug&&d("<div>").html("start at "+k+"px").prependTo(b.opts.debug),b.opts.eventsSettings.lastXYById[j]={x:k,y:l},m=k,a.msSetPointerCapture?a.msSetPointerCapture(j):"mousedown"==g.type&&1==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)&&(b.opts.eventsSettings.useSetReleaseCapture?
a.setCapture(!0):(document.addEventListener("mousemove",c,!1),document.addEventListener("mouseup",c,!1)));else if(g.type.match(/move$/i))b.opts.eventsSettings.lastXYById[j].x=k,b.opts.eventsSettings.lastXYById[j].y=l,e=k-m,j=20,Math.abs(e)>j?(0>e&&(j=-j),j=e-j,b.opts.debug&&d("<div>").html("move: "+j+"px").prependTo(b.opts.debug),b.getItemsContainer().css("left",j+"px")):e=0;else if(b.opts.eventsSettings.lastXYById[j]&&g.type.match(/(up|end|cancel)$/i)){b.opts.debug&&d("<div>").html("end").prependTo(b.opts.debug);
var n=setInterval(function(){!1==b.opts.busy&&clearInterval(n);0<e?b.goToPrevious():0>e&&b.goToNext();e=0},10);delete b.opts.eventsSettings.lastXYById[j];a.msReleasePointerCapture?a.msReleasePointerCapture(j):"mouseup"==g.type&&0==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)&&(b.opts.eventsSettings.useSetReleaseCapture?a.releaseCapture():(document.removeEventListener("mousemove",c,!1),document.removeEventListener("mouseup",c,!1)))}}}}var b=this;if(window.navigator.msPointerEnabled)a.addEventListener("MSPointerDown",
c,!1),a.addEventListener("MSPointerMove",c,!1),a.addEventListener("MSPointerUp",c,!1),a.addEventListener("MSPointerCancel",c,!1),"undefined"!=typeof a.style.msContentZooming&&(a.style.msContentZooming="none"),"undefined"!=typeof a.style.msTouchAction&&(a.style.msTouchAction="none"),b.opts.debug&&d("<div>").html("Using Microsoft pointer model").prependTo(b.opts.debug);else if(a.addEventListener)a.addEventListener("touchstart",c,!1),a.addEventListener("touchmove",c,!1),a.addEventListener("touchend",
c,!1),a.addEventListener("touchcancel",c,!1),a.addEventListener("mousedown",c,!1),a.setCapture&&!window.navigator.userAgent.match(/\bGecko\b/)&&(this.opts.eventsSettings.useSetReleaseCapture=!0,a.addEventListener("mousemove",c,!1),a.addEventListener("mouseup",c,!1)),b.opts.debug&&d("<div>").html("Using mouse model with capture").prependTo(b.opts.debug);else if(a.attachEvent&&a.setCapture)opts.eventsSettings.useSetReleaseCapture=!0,a.attachEvent("onmousedown",function(){c(window.event);return window.event.returnValue=
!1}),a.attachEvent("onmousemove",function(){c(window.event);return window.event.returnValue=!1}),a.attachEvent("onmouseup",function(){c(window.event);return window.event.returnValue=!1}),b.opts.debug&&d("<div>").html("Using legacy IE mode - mouse model with capture").prependTo(b.opts.debug);else return b.opts.debug&&d("<div>").html("Unexpected combination of supported features").prependTo(b.opts.debug),!1;var m,e;this.$el.data("obj",b);this.$el.trigger(b.opts.controlInitializedEventName);return!0};
this.EnsurePageXY=function(a){if("undefined"==typeof a.pageX)if(a.pageX=a.offsetX+documentToTargetDelta.x,a.pageY=a.offsetY+documentToTargetDelta.y,a.srcElement.offsetParent==target&&document.documentMode&&8==document.documentMode&&"mousedown"==a.type)a.pageX+=a.srcElement.offsetLeft,a.pageY+=a.srcElement.offsetTop;else if(a.srcElement!=target&&!document.documentMode||8>document.documentMode){for(var c=-2,b=-2,d=a.srcElement;null!=d;d=d.parentNode)c+=d.scrollLeft?d.scrollLeft:0,b+=d.scrollTop?d.scrollTop:
