...
a?d.goToNext():d.goToPrevious()},d.opts.animationDuration)};this.goToNext=function(){this.changeItem(!0)};this.goToPrevious=function(){this.changeItem(!1)};this.setEvents=function(a){function d(g){if(!("mousemove"==g.type&&0==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)))for(var f=g.changedTouches?g.changedTouches:[g],j=0;j<f.length;++j){var m=f[j],h="undefined"!=typeof m.identifier?m.identifier:"undefined"!=typeof m.pointerId?m.pointerId:1;b.EnsurePageXY(m);var n=m.pageX,m=m.pageY;if(g.type.match(/(start|down)$/i)){documentToTargetDelta=
b.ComputeDocumentToElementDelta(a);if(b.opts.eventsSettings.lastXYById[h]){delete b.opts.eventsSettings.lastXYById[h];b.opts.debug&&c("<div>").html("end").prependTo(b.opts.debug);break}b.opts.debug&&c("<div>").html("start at "+n+"px").prependTo(b.opts.debug);b.opts.eventsSettings.lastXYById[h]={x:n,y:m};p=n;e=m;a.msSetPointerCapture?a.msSetPointerCapture(h):"mousedown"==g.type&&1==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)&&(b.opts.eventsSettings.useSetReleaseCapture?a.setCapture(!0):(document.addEventListener("mousemove",
d,!1),document.addEventListener("mouseup",d,!1)))}else if(g.type.match(/move$/i))void 0!==b.opts.eventsSettings.lastXYById[h]&&(b.opts.eventsSettings.lastXYById[h].x=n,b.opts.eventsSettings.lastXYById[h].y=m,k=n-p,l=e-m,h=20,10<Math.abs(l)?(wait=0,0<l&&"up"!=b.opts.eventsSettings.yDirection?(l-=20,b.opts.eventsSettings.yDirection="down"):b.opts.eventsSettings.yDirection="up",!1==b.opts.busy&&(l+=b.getItem(b.opts.currentIndex).scrollTop(),e+=b.getItem(b.opts.currentIndex).scrollTop()),b.opts.busy=
!0,b.getItem(b.opts.currentIndex).scrollTop(l+wait),b.opts.debug&&c("<div>").html("going "+b.opts.eventsSettings.yDirection).prependTo(b.opts.debug),k=0):(Math.abs(k)>h?(0>k&&(h-=2*h),h=k-h,b.opts.debug&&c("<div>").html("move: "+h+"px").prependTo(b.opts.debug),b.getItemsContainer().css("left",h+"px")):k=0,l=0));else if(b.opts.eventsSettings.lastXYById[h]&&g.type.match(/(up|end|cancel)$/i)){b.opts.debug&&c("<div>").html("end").prependTo(b.opts.debug);0!=l&&(b.opts.eventsSettings.yDirection=null,b.opts.busy=
!1,l=0);var q=setInterval(function(){!1==b.opts.busy&&clearInterval(q);0<k?b.goToPrevious():0>k&&b.goToNext();k=0},10);delete b.opts.eventsSettings.lastXYById[h];a.msReleasePointerCapture?a.msReleasePointerCapture(h):"mouseup"==g.type&&0==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)&&(0<k&&(g.preventDefault&&g.preventDefault(),g.preventManipulation&&g.preventManipulation(),g.preventMouseEvent&&g.preventMouseEvent()),b.opts.eventsSettings.useSetReleaseCapture?a.releaseCapture():(document.removeEventListener("mousemove",
d,!1),document.removeEventListener("mouseup",d,!1)))}}}var b=this;if(window.navigator.msPointerEnabled)a.addEventListener("MSPointerDown",d,!1),a.addEventListener("MSPointerMove",d,!1),a.addEventListener("MSPointerUp",d,!1),a.addEventListener("MSPointerCancel",d,!1),"undefined"!=typeof a.style.msContentZooming&&(a.style.msContentZooming="none"),"undefined"!=typeof a.style.msTouchAction&&(a.style.msTouchAction="none"),b.opts.debug&&c("<div>").html("Using Microsoft pointer model").prependTo(b.opts.debug);
else if(a.addEventListener)a.addEventListener("touchstart",d,!1),a.addEventListener("touchmove",d,!1),a.addEventListener("touchend",d,!1),a.addEventListener("touchcancel",d,!1),a.addEventListener("mousedown",d,!1),a.setCapture&&!window.navigator.userAgent.match(/\bGecko\b/)&&(this.opts.eventsSettings.useSetReleaseCapture=!0,a.addEventListener("mousemove",d,!1),a.addEventListener("mouseup",d,!1)),b.opts.debug&&c("<div>").html("Using mouse model with capture").prependTo(b.opts.debug);else if(a.attachEvent&&
a.setCapture)opts.eventsSettings.useSetReleaseCapture=!0,a.attachEvent("onmousedown",function(){d(window.event);return window.event.returnValue=!1}),a.attachEvent("onmousemove",function(){d(window.event);return window.event.returnValue=!1}),a.attachEvent("onmouseup",function(){d(window.event);return window.event.returnValue=!1}),b.opts.debug&&c("<div>").html("Using legacy IE mode - mouse model with capture").prependTo(b.opts.debug);else return b.opts.debug&&c("<div>").html("Unexpected combination of supported features").prependTo(b.opts.debug),
!1;var p,e,k,l;this.$el.data("obj",b);this.$el.trigger(b.opts.controlInitializedEventName);return!0};this.EnsurePageXY=function(a){if("undefined"==typeof a.pageX)if(a.pageX=a.offsetX+documentToTargetDelta.x,a.pageY=a.offsetY+documentToTargetDelta.y,a.srcElement.offsetParent==target&&document.documentMode&&8==document.documentMode&&"mousedown"==a.type)a.pageX+=a.srcElement.offsetLeft,a.pageY+=a.srcElement.offsetTop;else if(a.srcElement!=target&&!document.documentMode||8>document.documentMode){for(var d=
-2,b=-2,c=a.srcElement;null!=c;c=c.parentNode)d+=c.scrollLeft?c.scrollLeft:0,b+=c.scrollTop?c.scrollTop:0;a.pageX=a.clientX+d;a.pageY=a.clientY+b}};this.NumberOfKeys=function(a){if(Object.keys)return Object.keys(a).length;var c=0,b;for(b in a)++c;return c};this.ComputeDocumentToElementDelta=function(a){for(var c=0,b=0;null!=a;a=a.offsetParent){if(!navigator.userAgent.match(/\bMSIE\b/)||document.documentMode&&!(8>document.documentMode)||!("relative"==a.currentStyle.position&&a.offsetParent&&"relative"==
...
2:this.opts.currentIndex+1);var f=this.getItem(b),k=2<this.totalNumberOfItems?this.getItem(d):void 0,l,g;!a&&f&&(f.css({left:-this.opts._item_width+this.opts.itemsLeftMarginPercent}),3<this.totalNumberOfItems&&(l=this.fixIndex(b-1),g=this.getItem(l),this.getItems().not(":eq("+this.opts.currentIndex+")").not(":eq("+b+")").not(":eq("+l+")").hide(),this.getItem(this.fixIndex(b-1)).css({left:-this.opts._item_width+this.opts.itemsLeftMarginPercent})));a&&k&&(k.css({left:2*this.opts._item_width+this.opts.itemsLeftMarginPercent}),
this.getItems().not(":eq("+this.opts.currentIndex+")").not(":eq("+d+")").hide());a={left:(a?"-=":"+=")+this.opts._item_width};f&&f.show();k&&k.show();var j=this;f&&f.animate(a,this.opts.animationDuration,this.opts.animationEasing,function(){j.opts.currentIndex=b;j.opts.busy=!1;g&&g.show();j.$el.trigger(j.opts.selectedItemChangedEventName)});k&&k.animate(a,this.opts.animationDuration,this.opts.animationEasing);e.animate(a,this.opts.animationDuration,this.opts.animationEasing);this.getItemsContainer().animate({left:"0px"},
...
1,j=this.getItemsContainer().position().top,a=f-j;e.height(a);this.getItemsContainer().height(a);this.totalNumberOfItems=this.getItems().length;var d=e.outerWidth()*(100-this.opts.itemsGapPercentage)/100;this.opts._item_width=d;e=Math.round(c(window).width()/100*this.opts.itemsLeftMarginPercent);this.opts.itemsLeftMarginPercent=e;this.getItems().each(function(b,e){c(e).height(a-(c(e).outerHeight()-c(e).height()));c(e).width(d-(c(e).outerWidth()-c(e).width()));1==b&&c(e).css({left:d});1<b&&c(e).hide()});
...
$.easing.easeOutExpo=function(c,n,e,f,j){return n==j?e+f:f*(-Math.pow(2,-10*n/j)+1)+e};