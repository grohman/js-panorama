...
b.NumberOfKeys(b.opts.eventsSettings.lastXYById))){g.preventDefault&&g.preventDefault();g.preventManipulation&&g.preventManipulation();g.preventMouseEvent&&g.preventMouseEvent();for(var e=g.changedTouches?g.changedTouches:[g],j=0;j<e.length;++j){var k=e[j],h="undefined"!=typeof k.identifier?k.identifier:"undefined"!=typeof k.pointerId?k.pointerId:1;b.EnsurePageXY(k);var l=k.pageX,k=k.pageY;if(g.type.match(/(start|down)$/i))documentToTargetDelta=b.ComputeDocumentToElementDelta(a),b.opts.eventsSettings.lastXYById[h]&&
(delete b.opts.eventsSettings.lastXYById[h],b.opts.debug&&c("<div>").html("end").prependTo(b.opts.debug)),b.opts.debug&&c("<div>").html("start at "+l+"px").prependTo(b.opts.debug),b.opts.eventsSettings.lastXYById[h]={x:l,y:k},f=l,a.msSetPointerCapture?a.msSetPointerCapture(h):"mousedown"==g.type&&1==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)&&(b.opts.eventsSettings.useSetReleaseCapture?a.setCapture(!0):(document.addEventListener("mousemove",d,!1),document.addEventListener("mouseup",d,!1)));
else if(g.type.match(/move$/i))b.opts.eventsSettings.lastXYById[h].x=l,b.opts.eventsSettings.lastXYById[h].y=k,m=l-f,h=20,Math.abs(m)>h?(0>m&&(h=-h),h=m-h,b.opts.debug&&c("<div>").html("move: "+h+"px").prependTo(b.opts.debug),b.getItemsContainer().css("left",h+"px")):m=0;else if(b.opts.eventsSettings.lastXYById[h]&&g.type.match(/(up|end|cancel)$/i)){b.opts.debug&&c("<div>").html("end").prependTo(b.opts.debug);var n=setInterval(function(){!1==b.opts.busy&&clearInterval(n);0<m?b.goToPrevious():0>m&&
b.goToNext();m=0},10);delete b.opts.eventsSettings.lastXYById[h];a.msReleasePointerCapture?a.msReleasePointerCapture(h):"mouseup"==g.type&&0==b.NumberOfKeys(b.opts.eventsSettings.lastXYById)&&(b.opts.eventsSettings.useSetReleaseCapture?a.releaseCapture():(document.removeEventListener("mousemove",d,!1),document.removeEventListener("mouseup",d,!1)))}}}}var b=this;if(window.navigator.msPointerEnabled)a.addEventListener("MSPointerDown",d,!1),a.addEventListener("MSPointerMove",d,!1),a.addEventListener("MSPointerUp",
...
(a?c("<div>").html("swipe left").prependTo(this.opts.debug):c("<div>").html("swipe right").prependTo(this.opts.debug));var d,b,f=this.getItem(this.opts.currentIndex);b=this.fixIndex(a?this.opts.currentIndex+1:this.opts.currentIndex-1);d=this.fixIndex(a?this.opts.currentIndex+2:this.opts.currentIndex+1);var e=this.getItem(b),g=2<this.totalNumberOfItems?this.getItem(d):void 0;if(!a&&e){e.css({left:-this.opts._item_width+this.opts.itemsLeftMarginPercent});var j=this.fixIndex(b-1),l=this.getItem(j);this.getItems().not(":eq("+
this.opts.currentIndex+")").not(":eq("+b+")").not(":eq("+j+")").hide();this.getItem(this.fixIndex(b-1)).css({left:-this.opts._item_width+this.opts.itemsLeftMarginPercent})}a&&g&&(g.css({left:2*this.opts._item_width+this.opts.itemsLeftMarginPercent}),this.getItems().not(":eq("+this.opts.currentIndex+")").not(":eq("+d+")").hide());a={left:(a?"-=":"+=")+this.opts._item_width};e&&e.show();g&&g.show();var k=this;e&&e.animate(a,this.opts.animationDuration,this.opts.animationEasing,function(){k.opts.currentIndex=
b;k.opts.busy=!1;l&&l.show();k.$el.trigger(k.opts.selectedItemChangedEventName)});g&&g.animate(a,this.opts.animationDuration,this.opts.animationEasing);f.animate(a,this.opts.animationDuration,this.opts.animationEasing);this.getItemsContainer().animate({left:"0px"},this.opts.anumationDuration,this.opts.animationEasing);this.getTitle().animate({marginLeft:-b*this.opts.titleAnimationStep+"%"},this.opts.animationDuration,this.opts.animationEasing);this.$el.animate({backgroundPositionX:b*this.opts.backgroundAnimationStep+
"%"},this.opts.animationDuration,this.opts.animationEasing);return!0}}l.prototype={init:function(){var c=this.$el;if(c.data("panorama"))return!1;c.attr("data-panorama",!0);this.prepare(c);return!0},prepare:function(f){var e=f.parent().height()+1;f.height(e);var j=this.getItemsContainer().position().top,a=e-j;this.getItemsContainer().height(a);this.totalNumberOfItems=this.getItems().length;var d=f.outerWidth()*(100-this.opts.itemsGapPercentage)/100;this.opts._item_width=d;f=Math.round(c(window).width()/
...
"-="+e+"px")})})}};c.fn.panorama=function(f){this.length&&this.each(function(){var e=new l(this,f);e.init();e.setEvents(this);e.show();c(this).data("panorama",e)})}})(jQuery);$.easing.easeOutExpo=function(c,l,f,e,j){return l==j?f+e:e*(-Math.pow(2,-10*l/j)+1)+f};