...
{init:function(){return this.$el?(this.$el.attr("data-panorama",!0),this.prepare(this.$el),!0):!1},prepare:function(e){var j=void 0!==c.mobile?e.parent().height()+1:c(window).height(),g=this.getItemsContainer().position().top,b=j-g;e.height(b);this.getItemsContainer().height(b);this.totalNumberOfItems=this.getItems().length;var q=e.outerWidth()*(100-this.opts.itemsGapPercentage)/100;this.opts._item_width=q;e=Math.round(c(window).width()/100*this.opts.itemsLeftMarginPercent);this.opts.itemsLeftMarginPercent=
e;this.getItems().each(function(d,a){c(a).height(b-(c(a).outerHeight()-c(a).height()));c(a).width(q-(c(a).outerWidth()-c(a).width()));1==d&&c(a).css({left:q});1<d&&c(a).hide()});2<this.opts.totalNumberOfItems&&(this.getLastItem().css({left:"-"+q+"px"}).show(),this.getItems().css("left","+="+e+"px"))},show:function(){var e=this;this.getItemsContainer().fadeIn(function(){var j=e.getItemsContainer().position().top;e.getItems().each(function(){c(this).css("height","-="+j+"px")})})}};c.fn.panorama=function(e){this.length&&
this.each(function(){var j=new q(this,e);j.init()&&(j.setEvents(this),j.show(),c(this).data("panorama",j))})}})(jQuery);$.easing.easeOutExpo=function(c,q,e,j,g){return q==g?e+j:j*(-Math.pow(2,-10*q/g)+1)+e};